...
int8_t FT8Play(const char* msg, float f0);
 
#define SCRATCH_BUFF_SIZE  4800
#define RECORD_BUFFER_SIZE  9600
#define PLAY_BUFFER_SIZE    3600
...
int main()
{
    ...
    if (BSP_AUDIO_IN_Init(BSP_AUDIO_FREQUENCY_32K, DEFAULT_AUDIO_IN_BIT_RESOLUTION, DEFAULT_AUDIO_IN_CHANNEL_NBR) == AUDIO_ERROR) {
        ...
        while(cnt < 832000) 
        {
            if(audio_rec_buffer_state != BUFFER_OFFSET_NONE)
            {
                if(audio_rec_buffer_state == BUFFER_OFFSET_HALF)
                {
                    ...
                    {
                        if(i % 10 == 0) audio[index++] = RecordBuffer[i] / 32768.0f;
                    }
                    cnt += RECORD_BUFFER_SIZE / 2;
                }
                else
                {
                    for(int i = RECORD_BUFFER_SIZE / 2; i < RECORD_BUFFER_SIZE; ++i)
                    {
                        if(i % 10 == 0) audio[index++] = RecordBuffer[i] / 32768.0f;
                    }
                    cnt += RECORD_BUFFER_SIZE / 2;
    ...
    //Encode Test
    const char *msg2send = "CQ K1A2B OL94";
    printf("Input messages are: %s\n", msg2send);
    FT8Play(msg2send, 1000);
    //idle
    while(1)
    {
        // Rest of the program
...
int8_t FT8Play(const char* msg, float f0)
{
    ...
    synth_fsk(tones, NN, f0, symbol_rate, symbol_rate, sample_rate, &audio[num_silence]);
    ...
    //Audio out test1
    if (BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_HEADPHONE, 100, BSP_AUDIO_FREQUENCY_12K) == AUDIO_ERROR) 
        printf("BSP_AUDIO_OUT_Init error\n");

    int i = 0, cnt = 0;
    BSP_AUDIO_OUT_SetAudioFrameSlot(CODEC_AUDIOFRAME_SLOT_02);
    audio_out_buffer_state = BUFFER_OFFSET_NONE;
    FillOutBuffer(&PlaybackBuffer[0], (int16_t*)&pcmdata[i], PLAY_BUFFER_SIZE);
    SCB_CleanDCache_by_Addr((uint32_t*)&PlaybackBuffer[0], PLAY_BUFFER_SIZE);
    if (BSP_AUDIO_OUT_Play((uint16_t *) &PlaybackBuffer[0], PLAY_BUFFER_SIZE * 2) == AUDIO_ERROR)
        printf("Play error\n");
    i += PLAY_BUFFER_SIZE >> 2;
    ...
    while(i < 180000)
    {
        if(audio_out_buffer_state != BUFFER_OFFSET_NONE)
        {
            if(audio_out_buffer_state == BUFFER_OFFSET_HALF)
                FillOutBuffer(&PlaybackBuffer[0], (int16_t*)&pcmdata[i], PLAY_BUFFER_SIZE >> 1);
            else
            {
                FillOutBuffer(&PlaybackBuffer[PLAY_BUFFER_SIZE >> 1], \
                (int16_t*)&pcmdata[i + (PLAY_BUFFER_SIZE >> 2)], PLAY_BUFFER_SIZE >> 1);
                i += PLAY_BUFFER_SIZE >> 1;
            }
            audio_out_buffer_state = BUFFER_OFFSET_NONE;
        }
    }
 ...
 */
static void CopyRecBuffer(int16_t *dstbuffer, int16_t *srcbuffer, uint16_t BufferSize)
{
